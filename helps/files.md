...
1. 同名文件清理：自动查找并删除指定目录中具有相同主文件名但不同扩展名的冗余文件
2. 双格式处理：支持自定义两种文件扩展名的配对关系（如DNG+JPG、RAW+JPEG等）
...
1. 目录：选择需要扫描处理的文件夹路径
2. 搜索后缀名：设置作为保留依据的文件扩展名（如"DNG"）
3. 待删除后缀名：设置需要清理的配对文件扩展名（如"JPG"）
4. 后缀名不区分大小写：自动忽略扩展名的大小写差异（如.jpg和.JPG视为相同）
...
该工具通过文件系统枚举获取目标目录下所有符合搜索后缀名的文件，针对每个文件提取主文件名后，检查是否存在同名但扩展名为待删除后缀名的配对文件；若存在则将该配对文件标记为待删除项，用户确认后执行物理删除操作。整个过程采用先扫描后执行的二阶段模式，实现专业摄影工作流中常见的原始格式与预览格式的自动化清理。
